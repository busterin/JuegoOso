...
/* ================== Overlay de orientación (bloquea solo durante la partida) ================== */
function isPortrait() {
...
/* ================== Obstáculos ================== */
const OB_MIN_DURATION = 2.8, OB_MAX_DURATION = 3.6;
const OB_MIN_DELAY    = 900, OB_MAX_DELAY    = 1700;
...
function scheduleNextObstacle(ms) {
  ...
  obstacleTimer = setTimeout(spawnObstacle, ms);
}
...
function doAttack() {
  ...
  // Usar offsets (coordenadas locales sin escala) para posicionar la espada
  ...
  // Hitbox en coordenadas de pantalla (rects)
  ...
  // Destello (usamos offsets para que no se desplace con la escala)
...
setInterval(() => {
  ...
  if (isColliding(player, obstacle)) {
    ...
    const now = performance.now();
    if (now < invulnerableUntil) return; // aún invulnerable tras golpe
